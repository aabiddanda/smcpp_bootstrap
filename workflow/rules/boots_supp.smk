...
rule create_bootstrap_files_supp:
    ...
    output:
        expand(VCF_PATH + 'smcpp_format/bootstrap/{{POP}}_t1_{{t1}}_knots_{{knots}}_filt_{{n}}/run_1/bootstrap_chr{i}.gz', i = CHROMS)
    run:
        smcpp_dir = VCF_PATH + 'smcpp_format/{wildcards.POP}/'
        out_dir = VCF_PATH + 'smcpp_format/bootstrap/{wildcards.POP}_t1_{wildcards.t1}_knots_{wildcards.knots}_filt_{wildcards.n}/run'
        shell('python3 src/bootstrap_smcpp.py --nr_bootstraps 1  --out_dir_prefix %s %s/*.smcpp.gz' % (out_dir, smcpp_dir))
