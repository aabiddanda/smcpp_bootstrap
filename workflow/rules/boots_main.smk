...
rule create_bootstrap_files:
    ...
    output:
        expand(
            VCF_PATH
            + "smcpp_format/bootstrap/{{POP}}_t1_{{t1}}_knots_{{knots}}_filt_{n}/bootstrap_chr{i}.gz",
            n=BOOTS,
            i=CHROMS,
        ),
    run:
        ...
        out_dir = (
            VCF_PATH
            + "smcpp_format/bootstrap/{wildcards.POP}_t1_{wildcards.t1}_knots_{wildcards.knots}_filt"
        )
        shell(
            "python3 src/bootstrap_smcpp.py --out_dir_prefix %s %s/*.smcpp.gz"
            % (out_dir, smcpp_dir)
        )
