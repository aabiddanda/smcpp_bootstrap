...
rule check_sfs:
    ...
    run:
        shell(
            "awk '$3 == \"DEEPSEQ\" {{print $1}} ' {input.samplist} > {output.focal_inds}"
        )
        shell(
            "awk '$3 != \"DEEPSEQ\" {{print $1}} ' {input.samplist} > {output.non_focal_inds}"
        )
        shell(
            "vcftools --gzvcf {input.vcf} --min-alleles 2 --max-alleles 2  --keep {output.non_focal_inds} --missing-site --stdout | awk -v OFS='\t' 'NR > 1 && $6 == 1 {{print $1,$2}}' > {output.non_focal_miss_sites} "
        )
        shell(
            "vcftools --gzvcf {input.vcf} --min-alleles 2 --max-alleles 2 --keep {output.focal_inds} --missing-site --stdout | awk -v OFS='\t' 'NR > 1 && $6 == 1 {{print $1,$2}}' > {output.focal_miss_sites} "
        )
        shell(
            "vcftools --gzvcf {input.vcf} --keep {output.focal_inds} --positions {output.non_focal_miss_sites} --counts2 --stdout > {output.sfs_nonfocal_miss}"
        )
        shell(
            "vcftools --gzvcf {input.vcf} --keep {output.non_focal_inds} --positions {output.focal_miss_sites} --counts2 --stdout > {output.sfs_focal_miss}"
        )
...
rule check_split_seeds:
    input:
        smcpp_input=expand(
            VCF_PATH_SANITY
            + "smcpp_format/split/{{POP1}}_{{POP2}}/{{POP1}}.{{POP2}}.chr{i}.split.smcpp.gz",
            i=CHROMS,
        ),
    ...
    run:
        ...
        data_dir = (
            "{VCF_PATH_SANITY}smcpp_format/split/{wildcards.POP1}_{wildcards.POP2}"
        )
        shell(
            "smc++ split --seed %d --ftol 0.0001  -o %s {input.smc_model1} {input.smc_model2} %s/*.smcpp.gz "
            % (random.randint(0, 1000), out_dir, data_dir)
        )
