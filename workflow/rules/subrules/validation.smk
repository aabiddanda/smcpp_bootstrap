...
rule simulate_CEU_demography:
    ...
    run:
        shell(
            "python src/sim_demo.py -n {wildcards.n} -m {wildcards.m} -o {output.tmp_vcf} -r {input.recomb_map}"
        )
...
rule conv_vcf2smc_1pop_sim:
    ...
    output:
        temp_pop_file=temp(
            "data/smcpp_input_sim/CEU_{m,\d+}/focal_{f,\d+}/ceu_chr{CHROM, \d+}_n{n,\d+}.filt.{i,\d+}.list"
        ),
            ...
            with open(input.contig_length, "r") as f:
                for line in f:
                    ...
                    if l_splt[0] == wildcards.CHROM:
                        ...
                        break
            shell(
                "smc++ vcf2smc --length {contig_len} -c 50000 {input.vcf} {output.smc_out} 1 CEU:{pop_str}"
            )
...
rule run_smcpp_estimate_sim:
    input:
        smc_files=expand(
            "data/smcpp_input_sim/CEU_{{m}}/focal_{{f}}/ceu_chr{CHROM}_n{{n}}.filt.{i}.smcpp.gz",
            CHROM=CHROMS,
            i=[x for x in range(FOCAL)],
        ),
    output:
    ...
    run:
        ...
        shell(
            "smc++ estimate -o data/smcpp_output_sim/CEU_n{wildcards.n}_m{wildcards.m}/focal_{wildcards.f}/t1_{wildcards.t1}_knots_{wildcards.knots}/ -v {mu} {n_t1} {n_knots} data/smcpp_input_sim/CEU_{wildcards.m}/focal_{wildcards.f}/ceu_chr[0-9]*_n{wildcards.n}.filt.*.smcpp.gz"
        )
