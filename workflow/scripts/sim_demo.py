...
def tennessen_et_al(n=100, recomb_map = None):
    ...
    population_configurations = [
        msp.PopulationConfiguration(
            sample_size=n, initial_size=N_CEU0),
    ]
    demographic_events = [
        msp.PopulationParametersChange(
            time=0, initial_size=N_CEU0, growth_rate=0.0195),
        msp.PopulationParametersChange(
            time=204, initial_size=19930.67, growth_rate=0.00307),
        msp.PopulationParametersChange(
            time=920, initial_size=3722, growth_rate=0.0),
        msp.PopulationParametersChange(
            time=2040, initial_size=28948, growth_rate=0),
        msp.PopulationParametersChange(time=5920, growth_rate = 0, initial_size=N_A)
    ]
    if recomb_map is not None:
        tree_sequence = msp.simulate(sample_size=n, recombination_map=recomb_map, 
                                    demographic_events=demographic_events, 
                                    mutation_rate=1.2e-8)
    else:
        tree_sequence =msp.simulate(sample_size=n, length=10e6, 
                                    demographic_events=demographic_events, 
                                    mutation_rate=1.2e-8)
...
def gravel_et_al(n=100, recomb_map = None):
    ...
    demographic_events = [
        msp.PopulationParametersChange(
            time=0, initial_size=N_CEU0, growth_rate=0.0038),
        msp.PopulationParametersChange(
            time=920, initial_size=3722, growth_rate=0.0),
        msp.PopulationParametersChange(
            time=2040, initial_size=28948, growth_rate=0),
...
if __name__ =='__main__':
    ...
    parser.add_argument('-m', '--model', required=True, default=1, type=int, help='Choosing the demographic model to use')
    parser.add_argument('-n', required=True, type=int, help='Diploid Sample Size' )
    parser.add_argument('-r', '--recomb', required=True, help='Recombination Map (Hapmap format)')
    parser.add_argument('-o', '--out', required=True, help='output VCF file for the variation data')
